...
/**
 * Returns various details for the given Trixta channel string
 * @param channel
 * @returns TrixtaChannelDetails
 */
export const parseTrixtaChannel = (
  channel: string,
): TrixtaChannelDetails | undefined => {
  const [, roleNameWithGroup] = channel.split(':');
  if (!roleNameWithGroup) return undefined;
  const [role, group] = roleNameWithGroup.split('[');
  return {
    role,
