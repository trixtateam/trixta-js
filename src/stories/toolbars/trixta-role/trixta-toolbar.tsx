...
import {
  IconButton, Icons, TooltipLinkList, WithTooltip
} from '@storybook/components';
import React, { useCallback } from 'react';
import { Provider } from 'react-redux';
import { useTrixtaData } from '../../../React/hooks/use-trixta-data/use-trixta-data';
import store from '../../store';
import { TOOL_ACTION_ID, TOOL_REACTION_ID, TOOL_ROLE_ID, TRIXTA_ACTION_KEY, TRIXTA_REACTION_KEY, TRIXTA_ROLE_KEY } from './constants';
export const TrixtaRoleToolbar = () => {
  ...
  const {setRole,roles,actionNameList,reactionNameList} = useTrixtaData({roleName:trixtaRoleName});
  ...
  return (
      ...
      <IconButton
        ...
        >
         {trixtaRoleName ? <><Icons icon='user' style={{ marginRight:6}} />{trixtaRoleName}</>:<Icons icon='user' />}
        </IconButton>
      </WithTooltip>
      {trixtaRoleName && <WithTooltip
        placement="bottom"
        trigger="click"
        tooltipShown
        closeOnClick
      ...
      <IconButton
        ...
        >
         {trixtaActionName ? <><Icons icon='arrowright' style={{ marginRight:6}} />{trixtaActionName}</>:<Icons icon='arrowright' />}
        </IconButton>
      </WithTooltip>}
      {trixtaRoleName && <WithTooltip
        placement="bottom"
      ...
      <IconButton
        ...
        >
         {trixtaReactionName ? <><Icons icon='arrowleft' style={{ marginRight:6}} />{trixtaReactionName}</>:<Icons icon='arrowleft' />}
        </IconButton>
      </WithTooltip>}
      </>
  )
