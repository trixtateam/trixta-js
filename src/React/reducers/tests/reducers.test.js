  ...
  describe('trixtaReducer', () => {
    ...
    it('should handle the updateTrixtaRoles action correctly', () => {
      const action = {
        data: {
          roles: [
            { name: 'viewer[d1be63be-c0e4-4468-982c-5c04714a2987]' },
            { name: 'guest[d1be63be-c0e4-4468-982c-5c04714a2987]' },
      ...
      expect(expectedResult.authorizingStatus[action.data.roles[2].name]).toEqual({ status: true });
    });
        ...
        it('should handle the updateTrixtaReactionResponse action for requestForResponse with ui mode replace correctly', () => {
          ...
          const expectedResult = produce(state, (draft) => {
            ...
            if (isExpired) {
              ...
              if (index !== -1) {
                draft.reactions[keyName].instances[TRIXTA_FIELDS.requestForResponse].splice(
                  index,
                  1,
                );
                draft.reactions[keyName].loadingStatus = {};
